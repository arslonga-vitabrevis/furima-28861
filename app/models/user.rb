class User < ApplicationRecord
  ...
  with_options presence: true do
    ...
    password_regexp = /(?=.*[a-zA-Z])(?=.*[0-9])[a-z0-9A-Z]{6,}/
    ...
    validates :password,             format: { with: password_regexp, message: "は6文字以上の半角英数字で入力してください。" }, length: { minimum:6 }
    validates :password_confirmation,format: { with: password_regexp, message: "は6文字以上の半角英数字で入力してください。" }, length: { minimum:6 }
